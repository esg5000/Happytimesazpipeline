...
exports.runFetchRestaurants = runFetchRestaurants;
/**
...
function readGpsCoordinates(raw) {
...
function extractSerpApiImageUrl(raw) {
    if (raw == null)
        return undefined;
...
function isRestaurantLike(r) {
    ...
    if (primary.includes('restaurant') ||
...
function stableDocumentId(r, searchCity) {
    ...
    const hex = crypto_1.default.createHash('sha256').update(key).digest('hex').slice(0, 32);
    return `restaurant-${hex}`;
}
async function fetchMapsPage(q, ll, start) {
...
async function syncRestaurantsForCity(client, searchCity, ll) {
    ...
    for (const r of ranked) {
        ...
        const slugCurrent = `${(0, slug_1.generateSlug)(`${title} ${searchCity}`)}-${slugTail}`.slice(0, 96);
        const doc = {
            _type: 'restaurant',
            _id,
            name: title,
        ...
        if (priceLevel !== undefined)
            doc.priceLevel = priceLevel;
        const placeId = asNonEmptyString(r.place_id);
        if (placeId)
            doc.googlePlaceId = placeId;
        if (thumbnailUrl)
        ...
        try {
            ...
            console.error(`[restaurants] Sanity upsert failed "${title}" (${searchCity}):`, e instanceof Error ? e.message : e);
        }
        await new Promise((r) => setTimeout(r, 150));
    }
    console.log(`[restaurants] ${searchCity}: done — created=${created}, updated=${updated}, candidates=${ranked.length}`);
 ...
 */
async function runFetchRestaurants(options) {
    validateEnv();
