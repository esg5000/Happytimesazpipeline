...
exports.researchTopicWithProgress = researchTopicWithProgress;
exports.researchTopic = researchTopic;
exports.factCheckArticleMarkdownAnthropic = factCheckArticleMarkdownAnthropic;
exports.appendSourcesSectionMarkdown = appendSourcesSectionMarkdown;
/**
 * Research agent: expands editor notes with OpenAI Responses API + hosted `web_search`,
 * at most two web-search turns (one targeted hint if present, plus one generic angle),
...
async function openaiResponses(params, timeoutMs = 180000) {
    const key = config_1.config.openai.apiKey;
    if (!key) {
    ...
    const res = await axios_1.default.post(OPENAI_RESPONSES_URL, body, {
        headers: {
...
function normalizeSourceRow(raw) {
    ...
    const relevanceScore = clampScore(typeof o.relevanceScore === 'number'
        ? o.relevanceScore
...
function mergeSourcesByUrl(rows) {
    const map = new Map();
    for (const s of rows) {
 ...
 */
function extractTargetedResearchAngles(notes) {
    const found = [];
    const seen = new Set();
    const push = (key, angle) => {
        if (seen.has(key))
            return;
        seen.add(key);
    ...
    while ((m = reSearchFor.exec(notes)) !== null) {
        ...
        if (sourceHint.length < 2 || infoHint.length < 2)
            continue;
    ...
    while ((m = reFindInfoOn.exec(notes)) !== null) {
        ...
        if (siteHint.length < 2)
            continue;
        push(`find-on:${siteHint.toLowerCase()}`, [
...
function meaningfulPlainTextLength(text) {
    return text.replace(/\s+/g, ' ').trim().length;
}
/**
 * Headless Chromium: rendered DOM text when static HTTP fetch yields little content.
 */
async function fetchPagePlainTextWithPlaywright(url) {
    ...
    try {
        ...
        const inner = await page.locator('body').innerText({ timeout: 10000 }).catch(() => '');
...
async function fetchPagePlainText(url) {
    let parsed;
    try {
    ...
    try {
        const res = await axios_1.default.get(href, {
            ...
            responseType: 'text',
            headers: {
                'User-Agent': 'Mozilla/5.0 (compatible; HappyTimesAZ-ResearchBot/1.0; +https://happytimesaz.com)',
    ...
    const axiosMeaningful = axiosText !== null && meaningfulPlainTextLength(axiosText) >= PAGE_FETCH_MEANINGFUL_MIN_CHARS;
...
async function enrichTopSourcesWithFetchedPageText(sources, topN) {
    if (sources.length === 0)
        return sources;
...
async function runWebResearchForQuery(notes, query) {
...
function buildEnrichedNotes(notes, sources) {
    const base = notes.trim();
    if (sources.length === 0) {
        return `${base}\n\nRESEARCH FINDINGS:\n(No external sources were returned. Try broader notes or check API access.)`;
    }
    const lines = sources.map((s) => `- [${s.relevanceScore}/10] ${s.title} (${s.url})\n  ${s.summary}`);
    return `${base}\n\nRESEARCH FINDINGS:\n${lines.join('\n')}`;
}
/**
 ...
 */
async function researchTopicWithProgress(notes, onProgress) {
    ...
    if (targetedAngles.length > 0) {
        console.log(`[researchAgent] Running ${targetedAngles.length} targeted source search(es) before generic queries.`);
        await Promise.all(targetedAngles.map(async (angle) => {
    ...
    const remainingGenericSearchCalls = Math.max(0, MAX_WEB_SEARCH_CALLS - targetedAngles.length);
...
/**
 * Runs at most one targeted source angle → query extraction → one generic web-search pass
 * (`web_search`, max two Responses calls with tools), merges deduplicated sources, fetches plain
 * text for the top two scored URLs (≤3000 chars each) when possible, and appends RESEARCH FINDINGS.
 */
async function researchTopic(notes) {
 ...
 */
async function factCheckArticleMarkdownAnthropic(bodyMarkdown, sources) {
...
${JSON.stringify(sources.slice(0, 25)).slice(0, 60000)}
...
- Do not repeat overlapping strings; longer substrings preferred over many tiny ones.`;
    ...
    for (const row of rawWarnings) {
        ...
        if (verbatim.length < 12 || verbatim.length > 400)
            continue;
        if (!trimmed.includes(verbatim))
            continue;
        warnings.push({
            verbatim,
...
function appendSourcesSectionMarkdown(bodyMarkdown, sources) {
