...
exports.scrapeDispensaries = scrapeDispensaries;
/**
...
const DEALS_URL_MAP = {
    ...
    'menu.thegooddispensary.com': 'https://menu.thegooddispensary.com/stores/the-good-dispensary/deals',
...
const COMMON_DEALS_PATHS = ['/deals', '/specials', '/promotions'];
...
function matchDealsUrlOverride(baseUrl) {
    let u;
    try {
        u = new URL(baseUrl);
    }
...
function getFinalUrlFromAxiosResponse(res, fallback) {
    const req = res.request;
    const ru = req?.res?.responseUrl;
    if (typeof ru === 'string' && ru.length > 0)
        return ru;
 ...
 */
async function resolveDispensaryBaseUrlAfterRedirects(url) {
    try {
        const res = await axios_1.default.head(url, { ...AXIOS_REDIRECT_OPTS });
        const finalUrl = getFinalUrlFromAxiosResponse(res, url);
        if (res.status >= 200 && res.status < 400 && finalUrl)
            return finalUrl;
    }
    catch {
...
async function findDealsUrl(baseUrl) {
...
async function screenshotHomepageLogo(page, baseUrl) {
...
async function scrapeDomainOnce(context, domain, representative, anyNeedsScrapedImage, groupSize) {
    ...
    try {
        ...
        if (/age[-_]?gate/i.test(baseUrl)) {
            const nameForLog = representative.name?.trim() || representative.slug || representative._id;
            console.log(`[dispensaryScraper] age gate detected for ${nameForLog} — skipping deals URL`);
        }
        else {
        ...
        let scrapedImageAssetId;
        if (!anyNeedsScrapedImage) {
            console.log(`[dispensaryScraper] logo already set for ${representative.name?.trim() || representative.slug || representative._id} — skipping image scrape`);
        }
        else {
            ...
            if (logoBuf) {
                try {
                    ...
                    scrapedImageAssetId = await (0, sanityPublisher_1.uploadImageBufferToSanity)(logoBuf, `dispensary-scraped-${safe}.png`);
                    console.log(`[dispensaryScraper]   uploaded scrapedImage asset: ${scrapedImageAssetId}`);
                }
                catch (e) {
                    console.warn(`[dispensaryScraper]   scrapedImage upload failed:`, e instanceof Error ? e.message : e);
                }
            }
            else {
 ...
 */
async function scrapeDispensaries() {
    ...
    try {
        ...
        for (const [domain, group] of byDomain.entries()) {
            ...
            const rep = group.find((r) => r.hasManualLogo === false) ??
                group[0];
            if (!rep)
                continue;
            const anyNeedsScrapedImage = group.some((r) => r.hasManualLogo === false);
            try {
                const { dealsUrl, scrapedImageAssetId } = await scrapeDomainOnce(context, domain, rep, anyNeedsScrapedImage, groupSize);
                const fields = {
                    dealsScrapedAt: new Date().toISOString(),
                };
                if (dealsUrl)
                    fields.dealsUrl = dealsUrl;
                ...
                failed += groupSize;
                console.error(`[dispensaryScraper] FAIL domain ${domain}:`, e instanceof Error ? e.message : e);
            }
        }
    }
    finally {
    ...
    console.log(`[dispensaryScraper] ========== end: ok=${ok}, skipped=${skipped}, failed=${failed}, total=${list.length} ==========`);
...
//# sourceMappingURL=dispensaryScraper.js.map