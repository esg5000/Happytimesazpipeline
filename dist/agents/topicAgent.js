...
const TOPIC_PROMPT_PATH = (0, path_1.join)(process.cwd(), 'prompts', 'topic.prompt.txt');
 ...
 */
async function generateTopics(count = 3, options) {
    const prompt = (0, fs_1.readFileSync)(TOPIC_PROMPT_PATH, 'utf-8');
    const topics = [];
    ...
    if (editorialNotes) {
        console.log(`[topicAgent] Applying editorial notes to each topic request (${editorialNotes.length} chars)`);
    }
    for (let i = 0; i < count; i++) {
        try {
            const lastSection = topics.length > 0 ? topics[topics.length - 1].section : undefined;
            // Pass context about previously generated topics to avoid repetition
            const topic = await generateSingleTopic(prompt, topics, editorialNotes, applyStyle, articleLength, articleTone, lastSection);
            topics.push(topic);
        }
        catch (error) {
 ...
 */
async function generateSingleTopic(systemPrompt, existingTopics = [], editorialNotes, applyDashboardArticleStyle = false, articleLength = articleStyle_1.DEFAULT_ARTICLE_LENGTH, articleTone = articleStyle_1.DEFAULT_ARTICLE_TONE, lastGeneratedSection) {
    const styleAppend = applyDashboardArticleStyle
        ? (0, articleStyle_1.buildIngestArticleStyleAppend)(articleLength, articleTone)
        : '';
...
//# sourceMappingURL=topicAgent.js.map