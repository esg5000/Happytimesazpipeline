...
exports.syncDispensariesToSanity = syncDispensariesToSanity;
 ...
 */
const ARIZONA_SEARCH_LOCATIONS = [
    // Phoenix metro
...
function dedupeKey(r) {
    if (r.place_id)
        return `pid:${r.place_id}`;
    const name = normalizeKeyPart(r.title || '');
    const addr = normalizeKeyPart(r.address || '');
    return `na:${name}|${addr}`;
}
function documentIdFromKey(dedupe) {
    const hex = crypto_1.default.createHash('sha256').update(dedupe).digest('hex').slice(0, 32);
    return `dispensary-${hex}`;
}
function looksLikeCannabisDispensary(r) {
    const blob = [
        r.title || '',
        (r.types || []).join(' '),
        r.type || '',
...
function inferCategories(r) {
    const blob = [
        ...
        .toLowerCase();
    const out = [];
    if (/\bmedical\b|mmj|medicinal|\bmed\s+marijuana/.test(blob))
        out.push('medical');
    if (/\brecreational\b|\badult[- ]?use|21\s*\+|rec\s+only/.test(blob))
        out.push('recreational');
    if (out.length === 0)
        out.push('medical', 'recreational');
    return [...new Set(out)];
}
function formatHours(r) {
    ...
    if (typeof r.hours === 'string' && r.hours.trim())
        return r.hours.trim();
    if (typeof r.open_state === 'string' && r.open_state.trim())
        return r.open_state.trim();
    return '';
}
function parseCityFromAddress(address, fallbackCity) {
    ...
    for (let i = parts.length - 1; i >= 0; i--) {
        const p = parts[i];
        if (/\bAZ\b|Arizona/i.test(p) && i > 0)
            return parts[i - 1] || fallbackCity;
    }
    if (parts.length >= 2)
        return parts[parts.length - 2] || fallbackCity;
    return fallbackCity;
}
/** Max SerpApi `start` offset (Google Maps ~20 results per page; cap per SerpApi docs). */
function maxStartOffset() {
 ...
 */
async function syncDispensariesToSanity() {
    ...
    console.log(`[dispensaries] ${ARIZONA_SEARCH_LOCATIONS.length} Arizona search locations (Phoenix metro + statewide); global dedupe by place_id or name+address`);
    for (const { name: cityName, ll } of ARIZONA_SEARCH_LOCATIONS) {
        ...
        for (let start = 0; start <= maxStart; start += 20) {
            ...
            let status;
            try {
                const res = await axios_1.default.get(SERPAPI_SEARCH, {
                    params: {
                        engine: 'google_maps',
                        type: 'search',
                        q,
                ...
                errors++;
                console.error(`[dispensaries] HTTP error city=${cityName} start=${start}:`, e instanceof Error ? e.message : e);
                break;
            }
            if (status !== 200 || data.error) {
                console.warn(`[dispensaries] city=${cityName} start=${start} http=${status} err=${data.error || 'n/a'}`);
                if (start === 0)
                    break;
                break;
            }
            ...
            for (const raw of locals) {
                if (!looksLikeCannabisDispensary(raw))
                    continue;
                ...
                if (thumb) {
                    try {
                        const assetId = await (0, sanityPublisher_1.uploadImageToSanity)(thumb, `dispensary-${_id.slice(0, 24)}.jpg`);
                        imageRef = {
                            _type: 'image',
                        ...
                        console.warn(`[dispensaries] Image upload failed "${title}":`, e instanceof Error ? e.message : e);
                    }
                }
                const slugTail = _id.replace(/^dispensary-/, '').slice(0, 10);
                const slugCurrent = `${(0, slug_1.generateSlug)(`${title} ${city}`)}-${slugTail}`.slice(0, 96);
                const doc = {
                    _type: 'dispensary',
                    _id,
                    name: title,
                ...
                if (raw.place_id)
                    doc.googlePlaceId = raw.place_id;
                if (imageRef)
                    doc.image = imageRef;
                try {
                    ...
                    errors++;
                    console.error(`[dispensaries] Sanity upsert failed "${title}":`, e instanceof Error ? e.message : e);
                }
            }
            await new Promise((r) => setTimeout(r, 250));
        }
    }
    console.log(`[dispensaries] ========== end: uniqueFound=${uniqueFound}, saved=${saved}, errors=${errors} ==========`);
