...
import { recordSyncRun, recordTopicDiscoveryDebugLog } from './syncRunLogger';
    ...
    if (command === 'discoverTopics') {
      ...
      void (async () => {
        ...
        try {
          ...
          // was added for.
          const syncRunId = await recordSyncRun({
          ...
          // Durable counterpart to the local-only /tmp shadow-mode JSON log
          // (topicDiscovery.ts's writeShadowLog, still written as-is) — same
          // near-deduped pre-cap pool data, queryable from Sanity regardless
          // of which host ran the job.
          if (syncRunId) {
            await recordTopicDiscoveryDebugLog({
              syncRunId,
              createdAt: new Date().toISOString(),
              nearDedupedPool: result.nearDedupedPool,
              alreadySeenSkips: result.alreadySeenSkips,
            });
          }
        } catch (err: unknown) {
          const msg = err instanceof Error ? err.message : String(err);
