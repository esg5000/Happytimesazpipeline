 ...
 * — sports-az already has its own per-team diversity cap
 * (SPORTS_ENTITY_MAX_PER_SLOT) and isn't exhibiting the discard problem,
 * and health-wellness-az's raw supply skews national/generic rather than
 * AZ-specific. The general pool is untouched as a constant — it's derived
 * as the remainder (see buildCappedStage0Pool) and grows from 12→28
 * automatically as a side effect of the reserve increases, not a separate
 * decision. Prior history: raised from 36 to 44 on 2026-09-14 specifically
 * to grow CANNABIS_RESERVED_SLOTS and LIFESTYLE_RESERVED_SLOTS 6→10 each
 * (+8 total); sports-az and health-wellness-az reserves were UNCHANGED,
 * and the 12-slot general pool was unchanged too — that raise only
 * enlarged the two reserves. Confirmed via investigation (2026-09-14, real
 * run + Sept 6 shadow log) that cannabis-az and lifestyle-az each had
 * 54-99 raw near-deduped candidates/day against only 6 reserved slots,
 * discarding 89-94% of raw supply purely on same-class recency —
 * including genuinely good, on-brand, AZ-specific candidates (e.g.
 * "Phoenix's best Labor Day weekend parties", "Phoenix Curaleaf workers
 * win first union contract") that lost only because they were a day or
 * two older than whatever published same-day. Even after the 6→10 raise,
 * cannabis-az/lifestyle-az raw supply (236/303 candidates/day as of
 * 2026-09-17) still dwarfed the 10-slot reserves, so this raise continues
 * the same fix rather than reopening the question. health-wellness-az was
 * deliberately left at 6: its raw supply skews national/generic (Harvard
 * Health, NYT wire content) rather than AZ-specific, so raising its slice
 * has lower expected value. sports-az was deliberately left at 6 too — it
 * already has its own per-team diversity cap (SPORTS_ENTITY_MAX_PER_SLOT)
 * from a prior fix and isn't exhibiting the same problem. Older history:
 * 24→36 (2026-09-04, uniform 1.5x scale) verified the cap itself, not
 * Stage 1's ~56-63% pass rate, was the bottleneck; 20→24 added
 * HEALTH_WELLNESS_RESERVED_SLOTS.
 */
export const STAGE1_CANDIDATE_CAP = 80;
 ...
 * Raised 10→20 on 2026-09-18 (alongside LIFESTYLE_RESERVED_SLOTS, alongside
 * STAGE1_CANDIDATE_CAP's 44→80 raise): cannabis-az raw supply (236
 * candidates as of 2026-09-17) still ran ~23.6:1 against the 10-slot
 * reserve, so the same discard problem persisted even after the prior
 * raise. sports-az/health-wellness-az were again not touched — see
 * STAGE1_CANDIDATE_CAP's comment for why.
 */
const CANNABIS_RESERVED_SLOTS = 20;
/**
 ...
 * touched in this raise — see STAGE1_CANDIDATE_CAP's comment for why.
 * Raised 10→20 on 2026-09-18 (alongside CANNABIS_RESERVED_SLOTS, alongside
 * STAGE1_CANDIDATE_CAP's 44→80 raise): lifestyle-az raw supply (303
 * candidates as of 2026-09-17) still ran ~30.3:1 against the 10-slot
 * reserve, the worst raw-supply-discard ratio of the four reserved
 * verticals, so the same discard problem persisted even after the prior
 * raise. sports-az/health-wellness-az were again not touched — see
 * STAGE1_CANDIDATE_CAP's comment for why.
 */
const LIFESTYLE_RESERVED_SLOTS = 20;
/**
...
const STAGE0_QUERIES: Stage0Query[] = [
  ...
  { query: 'Arizona outdoor recreation new trail opening', queryClass: 'national-with-potential-local-angle' },
  { query: 'new business opening Arizona this month', queryClass: 'national-with-potential-local-angle' },
  { query: 'Arizona news this weekend', queryClass: 'national-with-potential-local-angle' },
  { query: 'Arizona new attraction opening', queryClass: 'national-with-potential-local-angle' },
  ...
  { query: 'Scottsdale events this month', queryClass: 'lifestyle-az' },
  { query: 'Arizona pop-up event new', queryClass: 'lifestyle-az' },
  { query: 'Mesa Tempe events this weekend', queryClass: 'lifestyle-az' },
...
Separately from locality AND from the crime/tragedy check above, judge editorialFit: does this story fit an upbeat, "good vibes and wild nights" Phoenix lifestyle brand — food, nightlife, cannabis culture, events, health-wellness, sports? Set editorialFit to true if it fits that tone, false if it's off-brand even though it isn't tragedy (dry policy analysis, generic hard-news process stories, bureaucratic/regulatory coverage with no local color or lifestyle angle). This is a real, separate exclusion — a story can pass the crime/tragedy check (it's not a tragedy) and still fail this one because it's simply tonally wrong for the site.
   Genuinely useful civic/safety content — a NAMED, ISSUED alert such as a heat warning, monsoon alert, or public-safety advisory (see the weather-specific rule below for exactly what qualifies) — should still be marked true; this is about TONE FIT, not whether something counts as real news. Don't over-filter genuine advisories.
   Tiebreaker for advisory-shaped vs. incident-shaped stories: does this change what a reader does today, or is it just "something happened"? Forward-looking advisories that help a reader plan (a construction heads-up, a named heat/monsoon warning) = true. Static, past-tense incident reports with no forward planning value and no lifestyle angle (a standalone crash/traffic-backup story with nothing else to it) = false.
   Weather specifically, applying and sharpening the tiebreaker above: mentioning heat, humidity, storm chances, wind, or monsoon season is NOT by itself enough to earn true, and neither is "this is practical" or "this helps a reader plan" reasoning on its own — that reasoning describes both a real advisory and a routine forecast equally, so it cannot be what decides the call by itself. What decides the call is whether a specific, NAMED, ISSUED alert is the actual substance of the story: an Excessive Heat Warning, Heat Advisory, Monsoon/Dust Storm Warning, Flash Flood Watch/Warning, Red Flag Warning, or an equivalent official advisory actually issued by the National Weather Service, a county, or another authority — where the story reports what that specific alert covers, when it's in effect, and what residents should do. Without that named issued alert as the story's substance, editorialFit is false — a weather story being well-written, forward-looking, or framed as "here's what to expect" does not on its own satisfy this bar. Day-to-day condition reporting, "storm chances build/increase," humidity or heat trending up or down, multi-day forecasts, or any other restating of expected conditions with no named issued alert behind it is filler and false — regardless of how useful or plan-oriented its framing sounds. Default to false for any weather story where the headline/snippet does not name a specific issued alert.
